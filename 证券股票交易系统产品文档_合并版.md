...
本系统采用 **RBAC (基于角色的访问控制)** 模型,核心设计原则如下:

| 原则 | 说明 |
| --- | --- |
| **单角色原则** | 每个用户账号仅属于一个角色,不支持多角色叠加 |
| **无升级机制** | 用户角色由管理员分配,不支持账户自动升级或角色转换 |
| **权限静态绑定** | 权限与角色绑定,不直接分配给用户 |
| **券商隔离** | 用户仅能访问被分配的券商服务端 |
...
| 角色 | 权限范围 | 典型用户 |
| --- | --- |
| **系统管理员** | 全局管理权限:账号管理、券商节点审核、系统监控、日志审计 | 公司IT管理员 |
| **交易管理员** | 交易团队管理权限:交易员账号管理、交易限额配置、团队报表 | 交易团队负责人 |
| **交易员** | 交易操作权限:行情查看、下单、撤单、持仓查询 | 执行交易的交易员 |

##### 3.2.2.3 权限矩阵

| 功能模块 | 系统管理员 | 交易管理员 | 交易员 |
| --- | --- | --- | --- |
| **账户管理** |
| 创建/删除账号 | ✅ | ✅ (仅交易员) | ❌ |
| 修改账号信息 | ✅ | ✅ (仅交易员) | ❌ |
...
**绑定规则**:
- 管理员可为交易员分配 1~N 个券商服务端访问权限
- 交易员登录时只能连接到已绑定的券商服务端
- 未绑定券商的交易员账号无法执行交易操作
...
用户表(accounts)
  - account_id (主键)
  - username
  - role (系统管理员/交易管理员/交易员)
  - status (active/frozen)

券商绑定表(account_brokers)
  - account_id (外键)
  - server_id (外键,关联券商节点表)
  - bind_at (绑定时间)
  
券商节点表(registered_nodes)
  - server_id (主键)
  - node_name
  - region
...
```
用户登录 → 管理端验证账号 → 返回JWT Token(含role+broker_list)
         ↓
客户端携带Token连接券商服务端 → 券商服务端验证Token
         ↓
    检查server_id是否在broker_list中 → 是 → 建立连接
                                      → 否 → 拒绝连接(403)
```
