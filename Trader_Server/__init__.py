"""
Trader_Server — 交易服务子服务端

业务执行侧组件，负责：
  1. 向 Server_manager 完成注册审核（HTTP REST + SSE）
  2. 持续心跳保活（Bearer Token 认证）
  3. 提供 WebSocket 服务端供客户端连接（Client ↔ TS）
  4. 经济数据采集与业务处理

启动方式:
    python -m Trader_Server.main
    python Trader_Server/main.py
    uvicorn Trader_Server.main:app --host 0.0.0.0 --port 8900
