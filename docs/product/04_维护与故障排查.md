...
5. 启动 SM Caddy/SM，确认 `https://scjrdomain.com:4430/ping` 返回成功。
...
| 入口 | 本地服务 |
|---|---|
| `https://scjrdomain.com:4430` | SM `127.0.0.1:18800` |
| `wss://<TS domain>/ws` | TS `127.0.0.1:8900` |

SM 公网开放 TCP 8800 和 4430，TS 保持其公网 80/443。SM 18800、TS 8900、Caddy admin 2019/2020、IB Gateway 4001 不应开放公网。若临时直连测试改变了监听地址，测试结束后必须恢复回环监听和防火墙策略。
...
```powershell
Test-NetConnection 127.0.0.1 -Port 18800  # SM 本机应用
...
```powershell
curl.exe -I https://scjrdomain.com:4430/ping
```

`HEAD` 可能返回 405，因为 `/ping` 只允许 GET；这仍说明 TLS、域名和 Caddy 已经到达 SM。需要确认业务响应时使用 GET：

```powershell
curl.exe https://scjrdomain.com:4430/ping
```
...
SM Caddy 管理地址为 `127.0.0.1:2019`，TS Caddy 管理地址为 `127.0.0.1:2020`。Caddyfile 示例位于 `deploy/caddy/`。SM Caddy 对外使用 8800/4430，反代到 `127.0.0.1:18800`；TS Caddy 反代 `/ws` 到 8900，并只暴露受控的 TS 管理接口路径。

Caddy 的证书和配置状态分别位于 `Server_manager/caddy/` 与 `Trader_Server/caddy/` 下的 `data/`、`config/`。SM 使用非标准公网端口时，证书签发仍依赖 CA 的 80/443 挑战端口；如果这些端口无法开放，应保留已有证书数据，或在 SM 启动环境中同时配置 `SM_CADDY_CERT_FILE` 和 `SM_CADDY_KEY_FILE`。重新安装时如果删除这些目录，可能需要重新签发证书；正式维护应先备份并确认 DNS 已指向正确服务器。不要把 Caddy 证书、私钥和自动生成数据提交到 Git。
...
1. DNS 是否解析到 SM 服务器。
2. `curl.exe https://scjrdomain.com:4430/ping` 是否返回 `{"status":"pong"}`。
3. SM 应用是否监听 18800，Caddy 是否监听 8800/4430 并反代到正确地址。
